        ...
        if (count($all_posts) > 0) {
            ...
            foreach ($all_posts as $index => $post) {
                $posts_array[] = array(
                    ...
                    "country" => $post->post_content,
                    "city" => $post->post_name,
                    "age" => $post->post_modified,
                    "email" => $post->post_author,
                    "phone" => $post->post_date,
                );
            }
        }
...
function cb_show_data_list_table()
{
    ...
    $table->prepare_items();
    echo "<form method='post' name='frm_search_post' action='" . $_SERVER['PHP_SELF'] . "?page=cb-datatable'>";
    $table->search_box('search', 'search_post_id');
    echo "</form>";   
    $table->display();
